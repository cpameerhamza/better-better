...
export const signupSchema = Yup.object().shape({
    ...
    institute_contact_name: Yup.string().min(2, "Minimum two characters are required.").required("Please school contact name."),
    email: Yup.string().email('Invalid email address.').required("Please enter school's email."),
    institute_contact_email: Yup.string().email('Invalid email address.').required("Please enter school's email."),
    institute_contact_email_confirmation: Yup.string().required("Please re-enter email.").oneOf([Yup.ref("institute_contact_email"), null], "Email must match."),
    email_confirmation: Yup.string().required("Please re-enter email.").oneOf([Yup.ref("email"), null], "Email must match."),
    ...
    institute_contact_number: Yup.string().matches(phoneRegExp, 'Phone number is not valid').required("Please enter your number."),
});
...
export const elderSchema = Yup.object().shape({
    ...
    citize_email_confirmation: Yup.string().required("Please re-enter email.").oneOf([Yup.ref("email"), null], "Email must match."),
    dob: Yup.string().required("Select your birth year."),
    institute_id: Yup.string().required("Select your school."),
...
export const commonQuestionsSchema = Yup.object().shape({
    ...
    current_state: Yup.string().min(1, "Please select at least one state.").required("This field is required."),
...
export const contactSchema = Yup.object().shape({
    ...
    last_name: Yup.string().min(2, "Minimum two characters are required.").required("Please enter your last name."),
    email: Yup.string().email('Invalid email address.').required("Please enter your email."),
    contact_number: Yup.string().matches(phoneRegExp, 'Contact number is not valid').required("Please enter your number."),
    message: Yup.string().min(10, "Minimum ten characters are required.").required("Please enter your message."),
});
