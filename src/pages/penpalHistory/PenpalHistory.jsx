...
import {letter, penpalAvatar, searchMessage, smallDot, threeDots, doubleArrow} from "../../utils/SvgIcons";
import {NavLink} from "react-router-dom";
import {disconnectPenpal, getSinglePenpalHistory, myPenpals} from "../../redux/thunks/Thunks";
...
const PenpalHistory = () => {
    ...
    const getPenpalHistory = (penpalID, clickedPenpal) => {
        ...
        dispatch(getSinglePenpalHistory({"penpal_id": penpalID, "token": token, "user": user})).then((response) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        dispatch(myPenpals({token: token, search: searchTerm})).then((response) => {
                    ...
                    <div className="main__body-content">
                        <div className="penpalmessage d__flex d__flex-h-between cols__holder">
                           <div className="penpal-chat-history">
                               ...
                               <div className="chats-with-penpal-history">
                                   {
                                       ...
                                       penpals?.length ? penpals.map((penpal, key) => {
                                           return(
                                               <div onClick={(e) => {
                                                   getPenpalHistory(user?.role_type === 1 ? penpal?.elder_id : penpal?.student_id, penpal)
                                                   setSelectedPenpal(user?.role_type === 1 ? penpal?.elder_id : penpal?.student_id)
                                                   getPenpalImage(penpal)
                                               }} key={key} className={`penpal-card d__flex  d__flex-v-center d__flex-h-between ${penpalCard === penpal && "activedpenpal"}`}>
                                                    {
                                                   ...
                                                   <div className="penpal-info d__flex d__flex-v-center">
                                                       <div className="penpal-dp">
                                                           {
                                                            user?.role_type === 1 ?
                                                                <img src={`${process.env.REACT_APP_ASSETS_BASE_URL}/${penpal?.elder_data?.avatar}`} />
                                                                : penpalAvatar()
                                                           }
                                                       </div>
                                                       <div className="penpal-name">
                                                           <span>{user?.role_type === 1 ? penpal?.elder_data?.first_name + " " + penpal?.elder_data?.last_name : penpal?.student_data?.first_name}</span>
                                                           {
                                                               penpal?.deleted_at !== null || disconnectedPenpal === penpal ? <p>Disconnected</p> : ""
                                                           }
                                                       </div>
                                                   </div>
                                                        ...
                                                        <div className={`disconnect-with-penpal ${disconnect === penpal && "show-disconnect-bar"}`} onClick={() => toggleDisconnect(penpal)}>
                                                            {threeDots() }
                                                            {
                                                                ...
                                                                <div  className="disconnect">
                                                                    <button type="button" onClick={() => handleDisconnect(penpal)}>Disconnect</button>
                                                                </div>
                                                            }
                            ...
                            <div className={`penpal-message-box ${toogleMessageBox === true && "mobile-penpal-message-box"}`}>
                                {
                                    selectedPenpal !== null ?
                                        <>
                                            <div className="penpal-message-box-header d__flex d__flex-h-between d__flex-v-center">
                                                <div className="penpal-info d__flex  d__flex-v-center">
                                                    <div className="penpal-dp">
                                                        {
                                                            selectedPenpalImage ? <img src={`${process.env.REACT_APP_ASSETS_BASE_URL}/${selectedPenpalImage}`} /> : penpalAvatar()
                                                        }
                                                    </div>
                                                    <div className="penpal-name">
                                                        {
                                                            singlePenpalHistory && singlePenpalHistory.length ? singlePenpalHistory.map((record, key) => {
                                                                return(
                                                                    <span key={key}>{user?.role_type === 1 ? record?.elder_data?.first_name + " " + record?.elder_data?.last_name : record?.student_data?.first_name}</span>
                                                                )
                                                            }) : ""
                                                        }
                                                    </div>
                                                ...
                                                <div className="disconnect-penpal">
                                                    <button disabled={singlePenpalHistory?.[0]?.deleted_at !== null ? true : false} onClick={handleDisconnect}>
                                                        {
                                                ...
                                                <div className="mobile-penpal-menu">
                                                    <button type="button" onClick={()=> setToogleMessageBox(!toogleMessageBox)}> {doubleArrow()} </button>
                                                </div>
                                            </div>
                                                ...
                                                singlePenpalHistory && singlePenpalHistory.length ? singlePenpalHistory.map((record) => {
                                                        return(
                                                            record?.letters && record?.letters.length ? record?.letters.map((singleRecord, key) => {
                                                                return(
                                                                    <div key={key} className="message-with-penpal d__flex d__flex-h-between" >
                                                                        <div className="otheruser-penpal-end">
                                                                            ...
                                                                            <p className="uppercase">
                                                                                ...
                                                                                {smallDot()}
                                                                             <span className="send-date"> <SimpleDateTime timeSeparator=":" dateSeparator="-" showTime="1" showDate="0" meridians="1" format="DMY">{record?.created_at}</SimpleDateTime> </span>
                                                                            </p>
                                                                        </div>
                                                                        <div className="current-penpal-end">
                                                                            {letter()}
                                                                            <p>Penpal Letter {singleRecord.status === 1 ? "Received" : singleRecord.status === 2 ? "Pending" : "Sent"}!</p>
                                                                        </div>
                                                                    </div>
                                                                )
