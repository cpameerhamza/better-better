...
const StepTwo = ({formKeys, step}) => {
    ...
    const handleCheckboxes = (e, key, itemValue) => {
        if(e.target.checked){
            ...
            const filtered = selectedInterests.filter((same) => e.target.name !== same)
    ...
    const handleNext = () => {
        selectedInterests.length === 0 ? setInterestError("Please select at least one interest.") : setInterestError("");
    }
    ...
    return(
        ...
        <div className="forms-area">
            <form onSubmit={handleSearch} className="form__search">
                <button type="submit" className="form__search-btn">{searchIcon()}</button>
                <input type="text" placeholder="Search for interests" onChange={(e) => setSearchTerm(e.target.value)} />
            </form>
                        ...
                        <div className="signupform__step-two">
                            ...
                            <div className="user__interests selected__user-interest">
                                <div className="user__interest-box">
                                    ...
                                    <div className="user__interest-items d__flex gap-1">
                                        {
                                            selectedInterests && selectedInterests.length ? selectedInterests.map((tag, key) => {
                                                return(
                                                    <div key={key} className="selected-tag">{tag}</div>
                                                )
                                            }) : ""
                                        }
                                    </div>
                            ...
                            </div>
                            {
                                Object.keys(interests).map((int, key) => (
                                    <div key={key} ref={checkboxes} className="user__interests select__user-interest">
                                        <h4>{int}</h4>
                                        <div className="user__interest-items d__flex gap-1">
                                            {interests[int].map((item, id) => (
                                                <div key={id} className="user__interest-item">
                                                    <input
                                                        ...
                                                        name={item.label}
                                                        value={item.value}
                                                        onChange={(e) => {handleCheckboxes(e, key, item.value)}}
                                                    />
                                                    <label htmlFor={`sport-check-${item.value}`}>{item.label}</label>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            <p className="interest__error">{interestError !== null ? interestError : null}</p>
                        </div>
                    </div>
                </section>
                <div className="signup__form-submit">
                    <div className="submit__btn-container d__flex d__flex-h-between">
                        ...
                        <div className="multi-button">
                            ...
                            <button type="submit" onClick={handleNext} className="signup__next-btn">NEXT</button>
                        </div>
                    </div>
                </div>
            </form>
