...
const FindPenpals = () => {
    ...
    useEffect(() => {
        if(isFiltersApplied === true){
            dispatch(findPenpals({token: token, institute_id: appliedInstitute, interest: appliedInterests.join(' ') })).then((response) => {
                setLoading(false);
                setMatchedPenpals(response?.payload.data);
                response?.payload.data.length ? setEmpty(true) : setEmpty(false);
                    ...
                    <div className="main__body-content">
                        <div className="matches-holder">
                            <header className="header d__flex d__flex-v-center  d__flex-h-between">
                                <h3>Best Match</h3>
                                <button type="button" className="filter-match" onClick={showFilter}>{filtersIcon()} Filters</button>
                            </header>
                            <div className="matches-list">
                                {
                                    ...
                                    matchedPenpals && matchedPenpals.length ? matchedPenpals.map((penpal, key) => {
                                        return(
                                            <>
