...
const Settings = () => {
    ...
    const {values, handleSubmit, handleBlur, touched, handleReset, errors, handleChange, setFieldValue} = useFormik({
        initialValues,
        ...
        onSubmit: (values) => {
            ...
            dispatch(updateProfile({values, isChanged})).then((response) => {
                ...
                else if(response?.payload.message === "The avatar field must be a file of type: jpg, jpeg, png."){
                ...
                else if(response?.payload.message !== "Please enter a valid institute" && response?.payload.status === false){
    ...
    useEffect(() => {
        const interesetName = institutes && institutes[0].filter((filtered) => filtered.value === values?.institute_id).map((obj) => obj.label)
                        ...
                        <div className="profile__setting">
                            <div className="profile__setting-top d__flex d__flex-h-between">
                                ...
                                <div className="profile__setting-btn d__flex gap-1">
                                    <button className="setting__btn cancel__btn" onClick={handleReset} type="button">Cancel</button>
                                    <button type="submit" className="setting__btn save__btn" disabled={loading ? true : false}>Save Changes</button>
                                </div>
                            </div>
                            <div className="profile__setting-form">
                                <div>
                                    <div className="signup__form-one d__flex d__flex-h-between gap-2">
                                        ...
                                        <h4>Update Information</h4>
                                        {
                                            user?.role_type === 2 && !user?.isInstitute ?
                                                <div className="form__group full__field">
                                                    ...
                                                    {values?.avatar ? <PreviewPhoto file={values?.avatar} /> : <img src={`${process.env.REACT_APP_ASSETS_BASE_URL}/${user?.avatar}`} alt="#" /> }
                                        ...
                                        <div className={`form__group ${user?.role_type === 2 ? "half__field" : "full__field"}`}>
                                        ...
                                        <div className="form__group half__field">
                                            ...
                                            <div className="form__box"  ref={ref}>
                                                ...
                                                <input
                                                    ...
                                                    name="institute_id"
                                                    placeholder="Select Your Institute"
                                                    // value={institutes && institutes[0].filter((filtered) => filtered.value === values?.institute_id).label}
                                                    value={interestValue}
                                                    onChange={handleChange}
                                                    onBlur={handleBlur}
                                        ...
                                        <div className="form__group full__field">
                                            ...
                                            />
                                            <p className="error-msg">{errors.current_password && touched.current_password ? errors.current_password : null}</p>
